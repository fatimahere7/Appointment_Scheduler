...
// 	}
	function save_appointment(){
		...
		$check = $this->conn->query("SELECT * FROM `appointments` where ('".strtotime($date_sched)."' Between unix_timestamp(appointment_data) and unix_timestamp(DATE_ADD(appointment_date, interval 30 MINUTE)) OR '".strtotime($date_sched.' +30 mins')."' Between unix_timestamp(appointment_date) and unix_timestamp(DATE_ADD(appointment_date, interval 30 MINUTE))) ".($id >0 ? " and id != '{$id}' " : ""))->num_rows;
		$this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
...
switch ($action) {
	case 'save_appointment':
		echo  save_appointment();
	break;
	case 'multiple_action':
		echo  multiple_action();
	break;
	case 'save_sched_settings':
		echo save_sched_settings();
		break;
	default:
		// echo $sysset->index();
		break;
